...
def wafs_final_test(x_feature_tr, x_feature_ts, y_tr, y_ts, vectorizer, text, s_method, lamda=0.5):
    ...
    for i in range(len(initial_features)):
        ...
        new_s = estimate_s(x_feature_ts[best_features + [new_column]], y_ts, vectorizer, text,
                                           s_method=s_method)
    ...
    df = pd.DataFrame(data={'g':g, 's':s, 'gs':gs, 'lamda':lamdas})
    return df
...
x_feature_tr = x_feature_tr[PGD_features]
x_feature_ts = x_feature_ts[PGD_features]
sfs_df = wafs_final_test(x_feature_tr, x_feature_ts, y_tr, y_ts, tvec1, x_ts, PGD_only)
sfs_df.to_csv("PGDT_result.csv")

MagicWord_features = ['destdir', 'gtk', 'mnss_message_t', 'mnss_facility_ind_id', 'posting', 'rev', 'org', 'speakup', '2007', 'wrote',
 'cnn', 'code', 'data', 'source', 'sunny', 'perl', 'forecast', 'oneok', 'netflix', 'file',
'test', 'lists', 'thailand', 'linux', 'struct', 'lib', 'pmc', '0200', 'think', 'example',
'mailing', 'open', 'mean', 'goldfish', '3d', 'matrix', 'abbott', 'windows', 'pm', 'fog',
'topcoder', 'table', 'module', 'tf_mnss_message_category', 'smb', 'cbsnews', 'delivery', 'tcp', 'museum', 'cbs',
 'bush', 'stories', 'news', 'partition', 'get_lanman2_dir_entry', 'aumix', 'subplot', 'winding', 'gdata', 'r2html',
 'dataframes', 'ign', 'piano', 'smb_maxcnt', 'libreplace', 'sub', 'vtable', 'parrot', 'affordable', 'already',
 'default', '23', 'error', 'replace', '12', 'napster', 'suggestions', 'include', 'patent', 'countries',
'script', 'docs', 'increase', 'aids', 'ggplot2', 'summit', 'covers', 'amazing', 'sheldon',
'ctx_mem', 'salon', 'src', 'sizeof', 'article', 'carbon', 'for', 'tonight', 'kasger', 'continuations',
 '18098', 'satcon', 'dollars', 'alerts', 'buy', 'price', 'most', 'relations', 'www', 'correct',
 'log', 'create', 'talloc', 'intel', 'minutes', 'sqlplus', '07752694', 'utf', 'arm', 'suretymailings',
'adj', 'britney', 'sysvol', '40speedy', 'become', 'srcdir', 'llc', 'null', 'men', 'wind',
'gif', 'jeremy', 'givenchy', 'elite', 'thursday', 'smiles', 'plot', 'speed', 'bulgaria', 'userinput',
 'enum', 'inputs', 'interp', 'cabot', 'statements', 'experts', 'titannews', 'theft', 'newsbrief', 'cell',
'investorplace', 'config', 'page', 'srea', 'card', 'both', 'vector', 'speech', 'wednesday', 'boot',
'legend', 'îò', 'dell', 'recipients', 'casino', 'trying', 'import', 'lowest', 'netlogon', 'padding',
'mcse', 'million', 'base64', 'setup', 'pcwindows', 'glib', 'computer', 'license', 'but', 'his',
'bbc', 'preferences', 'argument', 'andy', 'petdance', 'latest', 'ip', 'week', 'de', 'too',
'mail', 'forwarded', 'content', 'line', '15', 'inline', 'sugar', 'meds', 'following', 'avcooper',
'lmer', 'ad0be', 'rolex', 'only', 'boundary', 'watches', 'june', '0d', 'options', 'see',
'release', 'should', 'jerry', 'tridge', 'product', 'hash', 'function', 'version', '13', 'length',
 'change', 'kate', 'transfer', 'library', 'td', 'results2', 'non', 'dc', 'radiation', 'manager',
'sorry', '54', 'rlapack', 'quintum', 'story', 'show', 'documentation', 'powermarketers', 'type', 'columns',
'eppg', 'imaging', 'donny', 'http', 'debian', 'install', 'changed', 'do', 'your', 'samba',
'update', 'cc', 'received', '56', 'digital', 'mhln', 'follow', 'clouds', 'company', 'patch',
'output', 'using', 'pmcs', 'pod', 'minimal', 'dept', 'david', 'configure', 'problem', 'products',
 '11', 'acls', 'corporation', 'over', 'minor', 'body', 'uwo', 'encoding', 'qualitystocks', 'added',
'int', 'long', 'pio', 'twinkle', 'hk', 'money', 'utc', 'lodrick', 'safe', 'weeks',
'drugs', 'sequestration', 'aspx', 'maximum', 'uranium', 'sendlen', 'pygame', 'ipr', 'beverage', 'rodale',
 'tidy', 'reform', 'uncensored', 'two', 'larry', 'be', '14', 'office', 'return', 'you',
 'our', 'case', 'list', 'rustypromdress', 'alert', 'samba_3_0_26', 'accuweather', 'laptop', 'investment', 've',
'printable', 'language', 'heimdal', 'producttestpanel', '06', 'ekiga', 'pdf', 'tid', 'ufsj', 'station',
'dq0uyvjaq8htmikjz3yh', 'alt', 'stay', 'profile', 'she', 'tdb', 'allison', 'bit', 'also', 'many',
'pills', 'utilities', 'cm', 'h323', 'homes', 'compiled', 'second', 'ap', 'rights', 'qmail',
'inc', 'us', 'ermx', 'girl', '52', 'dosage', 'tidy_args', 'multipart', 'reply', 'ktwarwic',
'fecund', 'jeqwkqomi78l5dcsqgylvyxkmjkqdiu', 'dhamma', 'imaging3', 'bwj', 'lottery', 'online',
'experience', 'tuesday', 'mlcr0s0ft', 'howstuffworks', 'irs', 'borse', 'lm', 'disposition', 'that', 'nan',
'to', 'capacity', 'run', 'dataset', 'viewing', 'project', 'braille', 'work', 'trunk', 'nbsp',
'samba4', 'someone', 'package', 'shareholder', 'rate', 'service', 'txt', 'printf', '16', 'essential',
'mudletta', 'quality', 'patrol', 'smp', '99', 'branches', 'pl', 'image', 'hi', 'swisscham',
'georgia', 'had', 'listinfo', 'their', 'question', 'beginners', 'get_string', 'gov', 'pugs', 'link',
'57', 'we', 'offer', 'expression', 'her', 'use', 'penis', 'com', 'hard', 'great',
'were', 'net', 'home', '_______________________________________________', 'false', 'web', 'testdb', 'done', 'have', 'up',
'csmo', 'ipaths', 'cid', 'karen', 'travlocks', 'd0', 'modified', 'thanks', 'anyone', 'state',
'mac', 'quoted', 'ctdb']
x_feature_tr = x_feature_tr[MagicWord_features]
x_feature_ts = x_feature_ts[MagicWord_features]
sfs_df = wafs_final_test(x_feature_tr, x_feature_ts, y_tr, y_ts, tvec1, x_ts, PGD_only)
